...
class SkillEmbedding(Base):
    ...
    # Dynamic type based on the database dialect
    def get_dimension_column(db_type):
        return Column(ARRAY(Float), nullable=False) if db_type == 'postgres' else Column(Text, nullable=False)

    dimension = get_dimension_column("sqlite")  # Set dynamically based on config
